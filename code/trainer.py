...
class Trainer(object):
    def __init__(self, model, optim, device, train_dataloader, val_dataloader, test_dataloader, data_name, split, ablation_mode='full', seed=42, **config):
        self.model = model
        self.optim = optim
        self.device = device
    ...
    def train(self):
        ...
        for i in range(self.epochs):
            ...
            self._save_val_metrics_to_csv()  # 实时保存验证集指标
            ...
            # 测试集评估（每轮都进行）
            test_auroc, test_auprc, test_f1, test_sensitivity, test_specificity, test_accuracy, test_loss, test_thred_optim, test_precision = self.test(
                dataloader="test")
            test_lst = ["epoch " + str(self.current_epoch)] + list(map(float2str, [test_auroc, test_auprc, test_f1,
                                                                                   test_sensitivity, test_specificity,
                                                                                   test_accuracy, test_thred_optim,
                                                                                   test_loss]))
            self.test_table.add_row(test_lst)

            # 实时更新三个表格文件
            self._save_table_to_file("val", self.val_table, "valid_markdowntable.txt")
            self._save_table_to_file("train", self.train_table, "train_markdowntable.txt")
            self._save_table_to_file("test", self.test_table, "test_markdowntable.txt")

            # 打印当前 epoch 的结果
            print('Validation at Epoch ' + str(self.current_epoch) + ' with validation loss ' + str(val_loss) +
                  " AUROC " + str(auroc) + " AUPRC " + str(auprc))
            print('Test at Epoch ' + str(self.current_epoch) + ' with test loss ' + str(test_loss) +
                  " AUROC " + str(test_auroc) + " AUPRC " + str(test_auprc) + " Sensitivity " + str(test_sensitivity) +
                  " Specificity " + str(test_specificity) + " Accuracy " + str(test_accuracy) + " Thred_optim " + str(
                test_thred_optim))
    ...
    def train_epoch(self):
        ...
        for i, (v_d, v_p, labels,mol_frames,d_kg,p_kg,input_ids,attention_mask) in enumerate(tqdm(self.train_dataloader)):
            self.step += 1
            v_d, v_p, labels,mol_frames,d_kg,p_kg,input_ids,attention_mask = v_d.to(self.device), v_p.to(self.device), labels.float().to(self.device),mol_frames.to(self.device),d_kg.to(self.device),p_kg.to(self.device),input_ids.to(self.device),attention_mask.to(self.device)
            self.optim.zero_grad()
            v_d, v_p, f, score, z_d, z_p = self.model(v_d, v_p,mol_frames,d_kg,p_kg,input_ids,attention_mask)
            # 修改：使用加权二元交叉熵损失
            if self.n_class == 1:
    ...
    def test(self, dataloader="test"):
        ...
        with torch.no_grad():
            model.eval()
            for i, (v_d, v_p, labels,mol_frames,d_kg,p_kg,input_ids,attention_mask) in enumerate(data_loader):
                v_d, v_p, labels,mol_frames,d_kg,p_kg,input_ids,attention_mask= v_d.to(self.device), v_p.to(self.device), labels.float().to(self.device),mol_frames.to(self.device),d_kg.to(self.device),p_kg.to(self.device),input_ids.to(self.device),attention_mask.to(self.device)
                
                v_d, v_p, score, att = model(v_d, v_p, mol_frames, d_kg, p_kg, input_ids, attention_mask, mode="eval")
